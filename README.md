<div align="center">

# Hi there, I'm Nyege Vincent Chibuzor 👋

### UI/UX Designer · Aspiring Frontend Developer · Based in Nigeria 🇳🇬

*I design digital experiences that are intuitive, beautiful, and built for real people.*

</div>

---

## 👨‍💻 About Me

I'm a passionate **UI/UX Designer** with hands-on experience designing user-centered digital products — and I'm currently expanding my skills into **Frontend Development** to bring my designs to life with code.

My design philosophy: every screen should feel effortless. If the user has to think too hard, the design hasn't done its job yet.

- 🎨 **Design tool:** Figma
- 🌱 **Currently learning:** HTML, CSS & JavaScript
- 🔍 **Looking for:** UI/UX or Frontend internship opportunities
- 🤝 **Open to:** Collaborations, feedback, and connecting with other creatives
- 📍 **Location:** Nigeria

---

## 🚀 Featured Project

### ✈️ Travel Naija — Nigeria Tour & Vacation Booking App

> *"Your comfort, safety, and experience — handled end to end."*

**Travel Naija** is a full-featured travel booking app concept designed for Nigerians who want to explore the beauty of all **36 states** of Nigeria. The app handles everything from flight bookings, hotel accommodation, to guided tours — so users can focus on the experience, not the logistics.

**Key features designed:**
- 🗺️ Browse destinations across all 36 Nigerian states
- ✈️ Flight booking & hotel lodging
- 🏖️ Curated tour packages
- 🔒 Safe, secure payment flow
- 👤 User-first onboarding and booking experience

**Tool used:** Figma

🔗 [**View Live Prototype →**](https://www.figma.com/proto/xyuoo3B1QuXr5Au7ko71rC/Travel-Naija-Website?node-id=0-1&t=6hGEuXnaEOXoQXnF-1)

---

## 🛠️ Skills & Tools

| Design | Development (Learning) |
|---|---|
| Figma | HTML5 |
| UI Design | CSS3 |
| UX Research | JavaScript (Beginner) |
| Prototyping | Responsive Design |
| Wireframing | |

---

## 📈 My Journey

```
UI/UX Designer ──────────────────────► Frontend Developer
     ✅ Figma                               🌱 HTML & CSS
     ✅ User Research                        🌱 JavaScript
     ✅ Prototyping                          🔜 React
     ✅ Product Thinking                     🔜 Tailwind CSS
```

---

## 🌍 Let's Connect

I'm actively looking for **internship opportunities** in UI/UX Design or Frontend Development. If you're hiring, collaborating, or just want to talk design — let's connect!

<div align="center">

[![Portfolio](https://img.shields.io/badge/Figma%20Portfolio-F24E1E?style=for-the-badge&logo=figma&logoColor=white)](https://www.figma.com/proto/xyuoo3B1QuXr5Au7ko71rC/Travel-Naija-Website?node-id=0-1&t=6hGEuXnaEOXoQXnF-1)

</div>

---

<div align="center">

*"Good design is not just what it looks like — it's how it works."*

⭐ **Open to internship & collaboration opportunities** ⭐

</div>
